---
name: html-prd-annotator
description: >-
  一键为当前项目的所有 HTML 原型页面植入一套交互式 PRD 点位打点与规格对照工具。包括唯一路径选择器、内联卡片编辑、Markdown 富文本渲染、物理备份导入导出以及对齐对焦光圈动画。
---

# HTML PRD 标注工具植入 Skill

## 概述
本 Skill 指引您（AI 编码助手）为一个静态/动态 HTML 原型项目自动化植入一整套**“交互式 PRD 点位打点与规格对照工具”**。该工具能让产品经理在浏览器里直接对页面元素进行数字标点、书写 Markdown 格式的 PRD 规则、下载备份，且能一键定位与闪烁闪动高亮该元素。

## 适用场景
* 用户提供了新画好的 HTML 页面原型。
* 用户作为产品经理，需要开始对当前界面的元素打点、做业务需求标注。

## ⚠️ 数据保护绝对原则 (Data Preservation Rule)
> [!IMPORTANT]
> **绝对禁止自动删除、篡改或重置用户的 PRD 标注数据文件！**
> 1. 当您对 HTML 页面进行二次功能优化、结构重构、样式修饰或任何代码修改时，**绝对不能重新初始化、清空或覆盖** `assets/js/prd-data-[文件名].js`。
> 2. 该数据文件是用户辛辛苦苦标注的业务规则与点位心血，是核心资产。
> 3. 您修改/优化代码时，只需触碰 HTML、CSS 或 `prd-pin-tool.js`，**严禁修改或删除任何 `assets/js/prd-data-*.js` 的内容**。

---

## 注入实施工作流

当用户要求“植入 PRD 标注工具”或“做需求标注”时，请依次执行以下 3 个步骤：

### 1. 创建页面专属初始预设数据文件
在 `assets/js/`（或合理的静态 JS 目录）下，为当前目录下的**每一个 HTML 文件**新建一个命名为 `prd-data-[文件名].js` 的专属数据文件。
内容声明为一个空的全局数组：
```javascript
window.INITIAL_PRD_DATA = [];
```
> 例如：
> * 针对 `index.html` -> 创建 `assets/js/prd-data-index.js`
> * 针对 `detail.html` -> 创建 `assets/js/prd-data-detail.js`

### 2. 创建核心交互控制文件 `prd-pin-tool.js`
在 `assets/js/` 下创建核心打点控制工具脚本 `prd-pin-tool.js`。完整源码实现如下（复制并直接写入该文件）：
...
### 3. 在所有 HTML 页面中注入脚本声明
定位到 HTML 页面底部 `</body>` 标签闭合前，依次注入两个 `<script>` 脚本，先加载对应的页面专属预设数据，再加载标注核心工具：
```html
<script src="./assets/js/prd-data-[本页面名字].js"></script>
...
## 常用避坑与审查指南
1. **不要加载全局通用的 prd-data.js**：以前可能存在一个统一的 `prd-data.js` 文件。必须改用独立解耦的单个 `prd-data-[本页面名字].js`，防止数据文件庞大冲突。
2. **选择器严禁简写**：在 `getUniqueSelector` 生成时，务必沿用包含父子链级的 `:nth-child` 绝对选择器路径，严禁使用类似 `document.querySelector('button')` 或简短的选择器，那会导致元素跳转紊乱。
3. **脉冲对焦动画配置**：确保 `flashTargetRing` 设置的脉冲动画运行且仅运行 1 次，并在 0.6 秒内自适应对焦完成并撤销光圈，不得残留在页面上。
